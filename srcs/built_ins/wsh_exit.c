...
void	wsh_exit_helper(t_wsh_list *wsh_list)
{
	if (wsh_list->ast_parsed->wsh_param || wsh_list->ast_parsed->wsh_arg)
	{
		...
		{
			if (ft_strncmp(wsh_list->ast_parsed->wsh_param[0], "9223372036854775807", 20) == 0)
				exit(255);
		}
		else
			if (ft_strncmp(wsh_list->ast_parsed->wsh_arg[0], "-9223372036854775808", 21) == 0)
				exit(0);
	}	
}
...
void	wsh_exit(t_wsh_list *wsh_list)
{
	...
	if (wsh_tab_length(wsh_list->ast_parsed->wsh_param) > 2 || wsh_tab_length(wsh_list->ast_parsed->wsh_arg) > 2)
	{
